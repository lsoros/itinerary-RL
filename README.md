...
Given an origin city, destination city, and carrier/departure day and time preferences, an agent must devise a sequence of one or more flight legs that will reliably get the traveler to their destination. A configurable reward signal guides agent learning, and an independent verifier checks the planned itinerary against a ground truth optimal itinerary. 

This environment was designed by hand and implemented via conversation with a mix of Cursor Grok 4.6, Claude Opus 5, GPT 5.6-Sol, Codex 5.3, and Claude Opus 4.5. Please note that Sol has very rarely been known to permanently erase data, so its use is not broadly advised.

The design specifications and contract for v1 can be found in [docs/spec.md](docs/spec.md), which also contains v2 planned extensions.
